...
*/
    ...
    public static void main(String[] args) {
        ...
        System.out.println("How Many different aspects are there to each test case: ");
        ...
        */
        ...
        if (numberOfAspects < 1)
        {
            ...
            for (int i = 1 ; i<= numberOfAspects ; i++)
            {
                ...
                TestCaseAspect aspect = new TestCaseAspect(); 
        ...
        //default return so code cmopiles
        //ArrayList<ArrayList> inputs = new ArrayList<ArrayList>();
        /*
        procedures.addAll(Arrays.asList(tmpProcedures));
        plan_variations.addAll(Arrays.asList(tmpRates));
        mmcp_participation_variations.addAll(Arrays.asList(tmpMMCPstatus));
        prior_auth_types.addAll(Arrays.asList(tmpPAdates));
        
        inputs.add(procedures);
        inputs.add(plan_variations);
        inputs.add(mmcp_participation_variations);
        inputs.add(prior_auth_types);
        */
        return null;
    }
    ...
    public static List<String> ComputeTestCases(List<List> inputs) {
        ...
        for (int i =0; i < procedures.size(); i++){
            for (int j = 0; j < plan_variations.size(); j++){
                for (int k = 0; k < mmcp_participation_variations.size(); k++){
                    for (int l = 0; l < prior_auth_types.size(); l++){
                        testCaseNumber++;
                        
                        testCases.add("Test Case: #" + testCaseNumber + ": Claim for procedure: \"" + procedures.get(i) + 
                            "\"\n\tfor a member with rate plan: \"" + plan_variations.get(j) + 
                            "\"\n\t\twith MMCP Participant Status of: \""+ mmcp_participation_variations.get(k) +
                            "\"\n\t\t\twith a PA that is \"" + prior_auth_types.get(l) +"\".\n");
                        } // Prior Authorization dates loop
                    } // MMCP Participation Types loop
                } // plan variations loop
            }// procedures loop
        */ // end of old hard coded loop
